...
export default function DocumentViewer({ result, file }: DocumentViewerProps) {
  ...
  const statusBadge = isHighConf ? (
    <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-pill text-sm font-medium bg-[#059652]/10 text-[#059652] border border-[#059652]/20">
      <span className="w-2 h-2 bg-[#059652] rounded-full"></span>
      High Confidence ({confPercent}%)
    </span>
  ) : result.unified_confidence >= threshold * 0.75 ? (
    <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-pill text-sm font-medium bg-[#ffc107]/10 text-[#b38600] border border-[#ffc107]/30">
      <span className="w-2 h-2 bg-[#ffc107] rounded-full"></span>
      Check Outputs ({confPercent}%)
    </span>
  ) : (
    <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-pill text-sm font-medium bg-[#df1529]/10 text-[#df1529] border border-[#df1529]/20">
  ...
  return (
    <div className="flex-[1.2] bg-white border-r border-gray-100 flex flex-col overflow-hidden shadow-medilab">
      {/* Header - MediLab Style */}
      <div className="px-5 py-4 border-b border-gray-100 bg-gray-50/30">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-10 h-10 bg-[#1977cc]/10 rounded-full flex items-center justify-center">
            <span className="text-xl">📄</span>
          </div>
          <h3 className="text-sm font-semibold text-[#2c4964] truncate flex-1 font-heading">
            {result.filename}
          </h3>
        </div>
        ...
        {fileUrl && fileType === 'pdf' ? (
          ...
          <div className="w-full h-full overflow-auto p-6 flex items-start justify-center">
            <img
              ...
              className="max-w-full rounded-lg shadow-medilab-lg bg-white"
            />
          </div>
        ) : (
          <div className="w-full h-full flex items-center justify-center">
            <div className="text-center text-[#444444]">
              ...
              <div className="text-base font-medium text-[#2c4964] font-heading">{result.filename}</div>
              <div className="text-sm text-[#768692] mt-2">
                {result.pages_processed} page{result.pages_processed !== 1 ? 's' : ''} processed
              </div>
            </div>
          </div>
